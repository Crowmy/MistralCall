...
def _get_history():
    ...
    for history_question in history_list:
        if history_question.startswith('#user'):
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        window['mistral_answer_label'].update(f"Answer [{model_size}]")
    ...
    elif event == 'HistoryList<DoubleClick>':
        selected_text = values['HistoryList'][0]
        show_popup_selected(selected_text)

    if is_loop_receive == 1:
        while is_receiving:
            update_response()
            time.sleep(.1)
        if not is_receiving:
            is_loop_receive = 2
    elif is_loop_receive == 2:
        window['HistoryList'].update(history_list)
        is_loop_receive = 0
